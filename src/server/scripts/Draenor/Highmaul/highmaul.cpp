                ...
                switch (m_Events.ExecuteEvent())
                {
                    ...
                    case eEvents::EventShieldCharge:
                    {
                        if (Unit* target = ObjectAccessor::GetUnit(*me, m_ChargeTarget))
                        {
                            ...
                            pos = target->GetFirstCollisionPosition(target->GetObjectScale(), l_O);
                            me->ClearUnitState(UnitState::UNIT_STATE_ROOT);
                            me->GetMotionMaster()->MoveCharge(pos.m_positionX, pos.m_positionY, pos.m_positionZ + target->GetObjectScale());
