...
struct npc_krikthik : public ScriptedAI
{
    ...
    void SelectNextWaypoint(float& x, float& y)
    {
        ...
        x = CenterPos.GetPositionX() + (me->GetObjectScale() + RADIUS_CIRCLE) * std::cos(actualAngle);
        y = CenterPos.GetPositionY() + (me->GetObjectScale() + RADIUS_CIRCLE) * std::sin(actualAngle);
    }
